...
effect_trait_matrix <- read.csv("data/Tropics & non-tropics/PCA_and_diet_data_temperate_Aug2022.csv")
...
for(i in c(1:length(studies))) {
  ...
  set.seed(0)
  dist.functional <- gawdis::gawdis(x = effect_trait_matrix_ss, w.type = "optimized", groups = c(1,2,3,4,5,5,5,5,5,5,5,5), opti.maxiter = 1000)
  ...
  for (j in 1:100) {
    ...
    for (k in 1:(nrow(com_matrix_study))) {
      ...
      ## order the potential numbers by the sensitivity scores of the species in the site
...
saveRDS(study_list_all, "data/Tropics & non-tropics/simulations_trait_nontropics.rds")
