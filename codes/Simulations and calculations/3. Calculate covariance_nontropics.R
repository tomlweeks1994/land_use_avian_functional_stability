...
response_trait_matrix <- read.csv("data/Tropics & non-tropics/response_trait_matrix_temperate_Aug2022.csv")


## Read in the neccessary datas
diversity <- readRDS("data/Tropics & non-tropics/diversity_combined_sites_temperate_Aug2022.rds")
com_matrix_all <- read.csv("data/Tropics & non-tropics/com_matrix_all_combined_studies_temperate_Aug2022.csv")
...
trait_matrix <- read.csv("data/Tropics & non-tropics/PCA_and_diet_data_temperate_Aug2022.csv")
...
write.csv(metrics_all, "data/Tropics & non-tropics/covariance_metrics_nontropics.R")
...
red_rich_stab_t <- read.csv("data/Tropics & non-tropics/nontropics_all_results_trait.csv")
red_rich_stab_vuln_t <- left_join(red_rich_stab_t, metrics_trait, by = "SSBS")

write.csv(red_rich_stab_vuln_t, "data/Tropics & non-tropics/analysis_data_trait_nontropics.csv")


### ALSO SAVE THESE INTO THE ABUNDANCE DATASET
## combine with red rich and stability results
red_rich_stab_a <- read.csv("data/Tropics & non-tropics/nontropics_all_results_abundance.csv")
red_rich_stab_vuln_a <- left_join(red_rich_stab_a, metrics_abundance, by = "SSBS")

write.csv(red_rich_stab_vuln_a, "data/Tropics & non-tropics/analysis_data_abundance_nontropics.csv")
