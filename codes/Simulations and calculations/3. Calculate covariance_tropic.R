...
write.csv(metrics_all, "data/Tropics & non-tropics/covariance_metrics_tropics.csv")
...
red_rich_stab_t <- read.csv("data/Tropics & non-tropics/tropics_all_results_trait.csv")
red_rich_stab_vuln_t <- left_join(red_rich_stab_t, metrics_trait, by = "SSBS")

write.csv(red_rich_stab_vuln_t, "data/Tropics & non-tropics/analysis_data_trait_tropics.csv")


### ALSO SAVE THESE INTO THE ABUNDANCE DATASET
## combine with red rich and stability results
red_rich_stab_a <- read.csv("data/Tropics & non-tropics/tropics_all_results_abundance.csv")
red_rich_stab_vuln_a <- left_join(red_rich_stab_a, metrics_abundance, by = "SSBS")

write.csv(red_rich_stab_vuln_a, "data/Tropics & non-tropics/analysis_data_abundance_tropics.csv")
