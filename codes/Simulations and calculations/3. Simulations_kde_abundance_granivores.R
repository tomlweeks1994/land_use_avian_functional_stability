...
avonet <- read.csv("../../Useful materials/AVONET1_BirdLife.csv")
diversity <- readRDS("Datasets/Aug2022/Main/diversity_combined_sites_Aug2022.rds")
diversity$Trophic_niche <- avonet$Trophic.Niche[match(diversity$Birdlife_Nominate, avonet$Species1)]
diversity <- diversity %>% filter(diversity$Trophic_niche == "Granivore")


com_matrix_all <- read.csv("Datasets/Aug2022/Granivores/com_matrix_all_combined_studies_granivores_Aug2022.csv")
...
effect_trait_matrix <- read.csv("Datasets/Aug2022/Granivores/PCA_and_diet_data_Aug2022_granivores.csv")
...
response_trait_matrix <- read.csv("Datasets/Aug2022/Granivores/response_trait_matrix_granivores_Aug2022.csv")
...
for(i in c(1:length(studies))) try({
  ...
  for (j in 1:25) {
    ...
    for (k in 1:(nrow(com_matrix_study))) {
      ...
      sp_probability_SSBS <- sp_probability_SSBS[match(colnames(com_matrix_study)[potential_numbers], row.names(sp_probability_SSBS)),]
...
saveRDS(study_list_all, "Datasets/Aug2022/Granivores/results_list_TPDs_abundance_weighted_granivores.rds")
